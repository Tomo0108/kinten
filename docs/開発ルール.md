...
│   ├── main.py
│   └── pdf_converter.py
├── distribution/           # 配布用ビルド成果物（統一された場所）
│   ├── backend/
│   │   └── kinten_backend.exe
│   ├── frontend/
│   │   ├── kinten.exe
...
├── templates/
├── .gitignore
├── build_all.bat           # 全体ビルドスクリプト
├── build_backend.bat       # バックエンドビルドスクリプト
├── build_frontend.bat      # フロントエンドビルドスクリプト
├── clean_build.bat         # ビルドクリーンアップスクリプト
├── create_distribution.bat # 配布用パッケージ作成スクリプト
...
- 2025-08-06 03:00: Excel転記機能とPDF変換機能の修正完了
   - Pythonコマンドの修正（Windows環境でpyコマンドを使用）
   - 自動転記機能の正常動作確認（CSV→Excel転記成功）
   - PDF変換機能の正常動作確認（Excel→PDF変換成功）
   - テスト用ファイルでの動作確認完了
   - 配布用ファイルの更新完了（最新版のkinten.exe）
- 2025-08-06 03:30: ファイル名文字化けによるシンタックスエラーの修正完了
   - Pythonスクリプト内での日本語ファイル名のエスケープ処理を改善
   - ファイルパスのエスケープ処理を強化（シングルクォートとダブルクォート両方に対応）
   - 文字化けによるシンタックスエラーの根本原因を解決
   - 配布用ファイルの更新完了（文字化け修正版のkinten.exe）
- 2025-08-06 04:00: 配布用パッケージの完全セットアップ完了
   - バックエンドPythonファイルの配布用ディレクトリへのコピー
   - Python依存関係の配布用環境へのインストール
   - 配布用ディレクトリでの動作確認（Excel転記・PDF変換両機能）
   - 使用方法とセットアップ手順を記載したREADME.mdの作成
   - 完全に動作する配布用パッケージの完成
- 2025-08-06 04:30: Unicodeエラーの修正完了
   - Pythonスクリプトのエンコーディング設定を改善（UTF-8宣言追加）
   - 環境変数の追加（PYTHONLEGACYWINDOWSSTDIO: utf-8）
   - バックエンドPythonファイルのエンコーディング宣言追加
   - 配布用ファイルの更新完了（Unicode対応版のkinten.exe）
   - 日本語ファイル名での正常動作確認完了

## ビルド構造ルール

### ビルド先の明確化
1. **統一された配布用フォルダ**: すべてのビルド成果物は `distribution/` フォルダに配置
2. **分離された構造**: バックエンドとフロントエンドは別々のサブフォルダに配置
3. **一時ビルドファイルの自動削除**: ビルド完了後に一時ファイルは自動的に削除

### ビルドスクリプトの役割
- `build_all.bat`: 全体ビルド（バックエンド + フロントエンド）
- `build_backend.bat`: バックエンドのみビルド
- `build_frontend.bat`: フロントエンドのみビルド
- `clean_build.bat`: すべてのビルドファイルを削除
- `create_distribution.bat`: 配布用ZIPパッケージを作成
...
- 生成物はGit管理外とする（`.gitignore`で `dist/**` と `packages/**` を除外）
- 変更を加えたら毎回ビルドを実施し、成果物は必ず `dist/` に集約する
- 配布物の `input/` と `output/` は空で配布し、個人情報ファイルは含めない
- `dist/` 配下に同名フォルダが二重にネストしないようにする（例: `dist/templates/templates` は禁止）

### Windows（開発者向けの基本手順）
- 一括ビルド: `build_all.ps1`
- 個別ビルド: `build_backend.ps1`（PyInstaller）、`build_frontend.ps1`（Flutter Windows Release）
- ビルド完了後の `dist/` 期待構成:
  - `kinten.exe`
  - `kinten_backend.exe`
  - `templates/`（雛形ファイルを含む）
...
### パッケージング（配布）
- 公式リリースZIP名（Windows）: `kinten_windows_<version>.zip`
- 配置先ディレクトリ規則:
  - Windows: `packages/windows/<version>/kinten_windows_<version>.zip`
  - macOS: `packages/mac/<version>/...`（ZIP名は別途指示に従う）
- ローカルでの汎用ZIP生成には `create_distribution.bat` を利用可（タイムスタンプ名）。公式リリースでは上記命名にリネームして配置する

### 注意事項（共通）
- `templates/勤怠表雛形_2025年版.xlsx` の存在を配布前に必ず確認する
- Windows版のPDF変換には Microsoft Excel（デスクトップ版）が必要
- Flutter/Excel など開発環境依存のある手順は、配布物では不要になるようビルドして同梱する
...
### macOS配布同期ルール（重要）
- macOS向けフロントエンドのビルド完了後は、毎回 `dist/` に同期すること。
  - `frontend/build/macos/Build/Products/Release/kinten.app` を `dist/macos/` へコピー
  - ルートの `backend/`, `templates/`, `input/`, `requirements.txt` を `dist/` へコピー
  - `dist/output/` を作成（空で可）
- 同期は以下のスクリプトで実行:
  ```bash
  bash scripts/sync_dist_macos.sh
  ```
- 依存導入（必要時・初回目安、macOSでは `pywin32` を除外）:
  ```bash
  cd dist
  python3 -m venv .venv
  source .venv/bin/activate
  pip install -r <(grep -v 'pywin32' requirements.txt) xlwings
  ```
- 理由: アプリは `kinten.app` から4階層上の `dist/` を起点に `backend/` などを探索し、`dist/.venv` を優先利用するため、毎回同期が必要。
